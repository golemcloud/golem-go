...
func WasiPollPollPollOneoff(in []uint32) []bool {
	var lower_in C.go_wasi_http_list_wasi_poll_poll_pollable_t
	if len(in) == 0 {
		lower_in.ptr = nil
		lower_in.len = 0
	} else {
	...
	defer C.go_wasi_http_list_wasi_poll_poll_pollable_free(&lower_in)
	var ret C.go_wasi_http_list_bool_t
...
func WasiIoStreamsRead(this uint32, len uint64) Result[WasiIoStreamsTuple2ListU8TWasiIoStreamsStreamStatusT, struct{}] {
	...
	var ret C.go_wasi_http_result_tuple2_list_u8_wasi_io_streams_stream_status_void_t
	...
	if ret.is_err {
		...
		lift_ret_ptr := *(*C.go_wasi_http_tuple2_list_u8_wasi_io_streams_stream_status_t)(unsafe.Pointer(&ret.val))
...
func WasiIoStreamsSkip(this uint32, len uint64) Result[WasiIoStreamsTuple2U64WasiIoStreamsStreamStatusT, struct{}] {
	...
	lower_this = lower_this_val
	lower_len := C.uint64_t(len)
	var ret C.go_wasi_http_result_tuple2_u64_wasi_io_streams_stream_status_void_t
	...
	if ret.is_err {
		...
		lift_ret_ptr := *(*C.go_wasi_http_tuple2_u64_wasi_io_streams_stream_status_t)(unsafe.Pointer(&ret.val))
...
func WasiHttpTypesFieldsGet(fields uint32, name string) [][]uint8 {
	...
	var ret C.go_wasi_http_list_list_u8_t
	C.wasi_http_types_fields_get(lower_fields, &lower_name, &ret)
	var lift_ret [][]uint8
	lift_ret = make([][]uint8, ret.len)
	if ret.len > 0 {
		for lift_ret_i := 0; lift_ret_i < int(ret.len); lift_ret_i++ {
			var empty_lift_ret C.go_wasi_http_list_u8_t
			lift_ret_ptr := *(*C.go_wasi_http_list_u8_t)(unsafe.Pointer(uintptr(unsafe.Pointer(ret.ptr)) +
				uintptr(lift_ret_i)*unsafe.Sizeof(empty_lift_ret)))
...
func WasiHttpTypesFieldsSet(fields uint32, name string, value [][]uint8) {
	...
	lower_name.len = C.size_t(len(name))
	defer C.go_wasi_http_string_free(&lower_name)
	var lower_value C.go_wasi_http_list_list_u8_t
	if len(value) == 0 {
		lower_value.ptr = nil
		lower_value.len = 0
	} else {
		var empty_lower_value C.go_wasi_http_list_u8_t
		lower_value.ptr = (*C.go_wasi_http_list_u8_t)(C.malloc(C.size_t(len(value)) * C.size_t(unsafe.Sizeof(empty_lower_value))))
		lower_value.len = C.size_t(len(value))
		for lower_value_i := range value {
			lower_value_ptr := (*C.go_wasi_http_list_u8_t)(unsafe.Pointer(uintptr(unsafe.Pointer(lower_value.ptr)) +
	...
	defer C.go_wasi_http_list_list_u8_free(&lower_value)
...
func WasiHttpTypesFieldsAppend(fields uint32, name string, value []uint8) {
	...
	lower_name.len = C.size_t(len(name))
	defer C.go_wasi_http_string_free(&lower_name)
	var lower_value C.go_wasi_http_list_u8_t
	if len(value) == 0 {
		lower_value.ptr = nil
		lower_value.len = 0
	} else {
	...
	defer C.go_wasi_http_list_u8_free(&lower_value)
...
func WasiHttpTypesFieldsEntries(fields uint32) []WasiHttpTypesTuple2StringListU8TT {
	...
	lower_fields_val := C.uint32_t(fields)
	lower_fields = lower_fields_val
	var ret C.go_wasi_http_list_tuple2_string_list_u8_t
	...
	if ret.len > 0 {
		for lift_ret_i := 0; lift_ret_i < int(ret.len); lift_ret_i++ {
			var empty_lift_ret C.go_wasi_http_tuple2_string_list_u8_t
			lift_ret_ptr := *(*C.go_wasi_http_tuple2_string_list_u8_t)(unsafe.Pointer(uintptr(unsafe.Pointer(ret.ptr)) +
				uintptr(lift_ret_i)*unsafe.Sizeof(empty_lift_ret)))
...
func WasiHttpTypesFinishIncomingStream(s uint32) Option[uint32] {
	...
	lower_s = lower_s_val
	var ret C.go_wasi_http_option_wasi_http_types_trailers_t
...
func WasiHttpTypesFinishOutgoingStream(s uint32, trailers Option[uint32]) {
	...
	lower_s_val = lower_s_val_val
	lower_s = lower_s_val
	var lower_trailers C.go_wasi_http_option_wasi_http_types_trailers_t
	if trailers.IsSome() {
...
func WasiHttpTypesIncomingRequestScheme(request uint32) Option[WasiHttpTypesScheme] {
	...
	lower_request_val := C.uint32_t(request)
	lower_request = lower_request_val
	var ret C.go_wasi_http_option_wasi_http_types_scheme_t
...
func WasiHttpTypesIncomingRequestConsume(request uint32) Result[uint32, struct{}] {
	...
	lower_request_val := C.uint32_t(request)
	lower_request = lower_request_val
	var ret C.go_wasi_http_result_wasi_http_types_incoming_stream_void_t
...
func WasiHttpTypesNewOutgoingRequest(method WasiHttpTypesMethod, path_with_query Option[string], scheme Option[WasiHttpTypesScheme], authority Option[string], headers uint32) uint32 {
	...
	defer C.go_wasi_http_option_string_free(&lower_path_with_query)
	var lower_scheme C.go_wasi_http_option_wasi_http_types_scheme_t
	if scheme.IsSome() {
	...
	defer C.go_wasi_http_option_wasi_http_types_scheme_free(&lower_scheme)
	...
	defer C.go_wasi_http_option_string_free(&lower_authority)
...
func WasiHttpTypesOutgoingRequestWrite(request uint32) Result[uint32, struct{}] {
	...
	lower_request_val := C.uint32_t(request)
	lower_request = lower_request_val
	var ret C.go_wasi_http_result_wasi_http_types_outgoing_stream_void_t
...
func WasiHttpTypesSetResponseOutparam(param uint32, response Result[uint32, WasiHttpTypesError]) Result[struct{}, struct{}] {
	...
	var lower_response C.go_wasi_http_result_wasi_http_types_outgoing_response_wasi_http_types_error_t
	...
	defer C.go_wasi_http_result_wasi_http_types_outgoing_response_wasi_http_types_error_free(&lower_response)
...
func WasiHttpTypesFutureIncomingResponseGet(f uint32) Option[Result[uint32, WasiHttpTypesError]] {
	...
	lower_f_val := C.uint32_t(f)
	lower_f = lower_f_val
	var ret C.go_wasi_http_option_result_wasi_http_types_incoming_response_wasi_http_types_error_t
...
func WasiHttpOutgoingHandlerHandle(request uint32, options Option[WasiHttpTypesRequestOptions]) uint32 {
	...
	lower_request = lower_request_val
	var lower_options C.go_wasi_http_option_wasi_http_outgoing_handler_request_options_t
	if options.IsSome() {
